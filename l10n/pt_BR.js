OC.L10N.register(
    ...
    "This app allows administrators to restrict login to their\nNextcloud server to specific IP ranges. Note that existing sessions will be kept\nopen.\n\nThe allowed IP ranges can be administrated using the OCC command line interface\nor graphical using the admin settings. If you plan to use the OCC tool, the\nfollowing commands would be applicable.\n\nTo whitelist `127.0.0.0/24`:\n\n- `occ config:app:set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/24`\n\nTo whitelist `127.0.0.0/24` and also `192.168.0.0/24`:\n\n- `occ config:app:set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/24,192.168.0.0/24`" : "Este aplicativo permite aos administradores restringir o login ao seu\nNextcloud para intervalos de IP específicos. Observe que as sessões existentes\nserão mantidas abertas.\n\nOs intervalos IP permitidos podem ser administrados pela de linha de comando OCC\nou graficamente nas configurações do administrador. Se usar a ferramenta OCC, o\nos seguintes comandos seriam aplicáveis.\n\nPara colocar `127.0.0.0/24` na whitelist:\n\n- `occ config:app:set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/24`\n\nPara colocar `127.0.0.0/24` e `192.168.0.0/24` na whitelist:\n\n- `occ config:app:set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/24,192.168.0.0/24`",
