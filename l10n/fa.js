OC.L10N.register(
    ...
    "This app allows administrators to restrict login to their\nNextcloud server to specific IP ranges. Note that existing sessions will be kept\nopen.\n\nThe allowed IP ranges can be administrated using the OCC command line interface\nor graphically using the admin settings. If you plan to use the OCC tool, the\nfollowing commands would be applicable.\n\nTo whitelist `127.0.0.0/24`:\n\n- `occ config:app:set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/24`\n\nTo whitelist `127.0.0.0/24` and also `192.168.0.0/24`:\n\n- `occ config:app:set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/24,192.168.0.0/24`" : "این برنامه به سرپرستان اجازه می دهد تا ورود به سیستم سرور خود را در محدوده IP خاص محدود کنند. توجه داشته باشید که جلسات موجود حفظ خواهد شد. محدوده IP مجاز می تواند با استفاده از رابط خط فرمان OCC - بصورت گرافیکی و با استفاده از تنظیمات مدیر اداره شود. اگر قصد استفاده از ابزار OCC را دارید ، دستورات پیروی قابل اجرا خواهد بود. \"لیست سفید\" 127.0.0.0/24`:- `occ config: app: set limit_login_to_ip whitelisted.ranges --value 127.0.0.0/ 24` برای لیست سفید `127.0.0.0 / 24` و همچنین` 192.168.0.0/24`:- `occ config: app: set limit_login_to_ip whitelisted.ranges --value 127.0.0.0 / 24،192.168.0.0 / 24`",
